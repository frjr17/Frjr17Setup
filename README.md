...
- `mutterNightLight.sh` — Rebuilds Fedora's `mutter` package with the identical-monitor Night Light fix in `patches/`, and installs it. Two identical monitors generate the same colord profile key, upstream rejects the second request with *"Profile generation already in progress"*, and that monitor's CRTC never gets a gamma LUT — so Night Light tints only one screen ([mutter#4736](https://gitlab.gnome.org/GNOME/mutter/-/issues/4736)). The build starts from the stock source RPM of whatever version dnf currently offers, so a `dnf upgrade` that replaces the patched build is repaired by re-running this. `--check` exits 1 when a rebuild is due, for a timer or a post-upgrade hook. Not part of `setup.sh`: it's specific to this laptop's dual identical monitors, takes several minutes, and should be deleted once the fix ships in Fedora.
